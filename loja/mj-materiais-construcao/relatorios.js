...
async function verificarAcesso() {
    ...
    // Perfis que podem acessar relatórios
...
function carregarVendedoresFiltro() {
    ...
    vendas.forEach(v => {
        if (v.vendedor_nome) {
            vendedores.add(v.vendedor_nome);
        } else if (v.vendedor?.nome) {
            vendedores.add(v.vendedor.nome);
        }
    });
...
function renderizarTabelaVendas() {
    ...
    paginados.forEach(v => {
        ...
        let statusClass = '';
        let statusText = v.status || 'concluida';
        if (statusText === 'concluida') statusClass = 'concluida';
        else if (statusText === 'cancelada') statusClass = 'cancelada';
...
window.verDetalhesVenda = async function(vendaId) {
    try {
        ...
        const dataVenda = venda.data_venda?.toDate?.() || new Date(venda.data_criacao || 0);
                        ...
                        <tbody>
                            ${vendasFiltradas.map(v => `
                                <tr>
                                    ...
                                    <td>${(v.data_venda?.toDate?.() || new Date(v.data_criacao || 0)).toLocaleString('pt-BR')}</td>
